...
from graded_common import (
    ...
    plot_spectrum, plot_tx_waveform, prbs_bits, save_wav,
)
...
def main() -> int:
    ...
    tx_meta = {"seed": args.seed, "n_bits": args.n_bits, "gain": args.gain, **meta}
    ...
    print(f"[tx] mode={args.mode} requested_rate={meta['bit_rate_requested']:.1f} actual_rate={meta.get('actual_bit_rate', meta.get('actual_rate', meta['bit_rate_requested'])):.1f} n_bits={args.n_bits} duration={len(wave)/FS:.3f}s gain={args.gain:.2f}")
    if args.no_play:
        return 0
    import sounddevice as sd
