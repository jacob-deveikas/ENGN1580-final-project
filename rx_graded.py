...
from graded_common import (
    ...
    plot_constellation, save_wav, bitstream_hash, prbs_bits,
)
...
def estimate_timeout(mode: str, rate: float, n_bits: int) -> float:
    # Include synchronization/training overhead, not just payload duration.
    if mode == "qpsk":
        return max(5.0, 0.8 + (n_bits + 192) / max(rate, 1) + 3.0)
    if mode == "cdma":
        return max(8.0, 0.8 + (n_bits + 48) / max(rate, 1) + 3.0)
    return max(8.0, 0.8 + (n_bits + 48) / max(rate, 1) + 3.0)
...
def main() -> int:
    ...
    expected_bits_for_hash = __import__("graded_common").prbs_bits(args.n_bits, args.seed)
    if args.verbose:
        print(f"[rx] expected seed={args.seed} n_bits={args.n_bits} bit_hash={__import__('graded_common').bitstream_hash(expected_bits_for_hash)}")
